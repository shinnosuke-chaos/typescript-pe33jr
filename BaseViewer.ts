...
export default class BaseViewer {
  ...
  constructor(private readonly options?: ViewerOptions) {
    ...
    // front view
    this.camera.position.copy(this.getOptions('camera.position'));
    this.camera.up = new Vector3().copy(this.getOptions('camera.up'));
    this.camera.lookAt(new Vector3().copy(this.getOptions('camera.lookAt')));
  ...
  getOptions(key) {
    const result = key.split('.').reduce(
      (acc, cur) => {
        acc.acc = acc.acc[acc.cur] || defaultViewerOptions[cur];
        acc.cur = acc.acc[cur];
        return acc;
      },
      { cur: undefined, acc: this.options }
    );
    return result.cur;
  }
